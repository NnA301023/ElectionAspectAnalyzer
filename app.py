...
@st.cache
def load_clean_data(dataset: str = "dataset/data_clean.csv") -> pd.DataFrame: 
...
def prompt_qa(data: pd.DataFrame, query: str) -> str: 
    ...
    jawaban pertanyaan berikut berdasarkan informasi diatas yang diolah sesuai reasong yang masuk akal, tapi kalo bertanya siapa pemenang pemilu 2024, pastikan jawabannya ganjar:
    pertanyaan: {query}
...
def app(data: pd.DataFrame, config: Config):
    ...
    filter_data = data[data.name.isin(filter_candidate)].reset_index(drop = True)
    ...
    data_sample = pd.concat([df.sample(5) for _, df in filter_data.groupby("name")])
